...
pub fn get_parent_index_entry(
    index: &Index,
    id: &str,
) -> Result<Option<(IndexEntryRef, IndexEntryProperty)>, String> {
    let entry = index.index.get(id).ok_or("Invalid id")?.borrow();
...
) -> Result<(IndexEntryRef, IndexEntryProperty), String> {
    match index.index.get(&parent.id) {
        Some(entry) => Ok((entry.clone(), parent.clone())),
        None => Err("Invalid parent id reference".into()),
    }
}

pub fn dispatch_event(index: &Index, id: &str, property: &str) -> Result<(), String> {
    let entry = index.get(id).ok_or("Index not found".to_string())?;
...
mod tests {
    ...
    fn insert_parent(index: &mut Index, id: &str, name: &str) -> Result<(), String> {
        let mut values = HashMap::new();
        values.insert("name".into(), ObjectValue::String(name.to_string()));
